...
class DataTrainingArguments:
    ...
    dataset_dir: Optional[str] = field(
        default=None, metadata={"help": "The input data directory (saved via save_to_disk)."}
    )
    ...
    early_stopping_patience: Optional[int] = field(
        default=15,
        metadata={
            "help": "Early stopping patience. This argument will be passed to ``EarlyStoppingCallback``."
        },
    )
    wandb_run_id: Optional[str] = field(
        default=None,
        metadata={"help": "Wandb run id to resume training."},
    )
    ...
    def __post_init__(self):
        if self.dataset_name is None and self.train_file is None and self.validation_file is None and self.dataset_dir is None:
            raise ValueError(
                "Need either a dataset name, dataset directory or a training/validation file.")
        else:
...
def main():
    ...
    def preprocess_function(examples):
        context = examples[context_column_name]
        answer = examples[answer_column_name]
        question = examples[question_column_name]

        inputs = [format_inputs(ctx, ans) for ctx, ans in zip(context, answer)]
        ...
        with tokenizer.as_target_tokenizer():
            labels = tokenizer(question, max_length=max_target_length,
                               padding=padding, truncation=True)
    ...
    def compute_metrics(eval_preds):
        ...
        result["bertscore"] = np.mean(F1.tolist())
